	...
	if data != 'OK':
		...
		if data != 'OK':
			...
			while (plot_close_event == 0):
				...
				# input('[ Cliente ]-$ Presionar \"enter\" para mandar un keep alive')
				...
				print("Recibi {}".format(data_sensor))	
				...
				accel_x[-1] = int(data_sensor[0])
				accel_y[-1] = int(data_sensor[1])
				accel_z[-1] = int(data_sensor[2])
				gyro_x[-1] 	= int(data_sensor[3])
				gyro_y[-1] 	= int(data_sensor[4])
				gyro_z[-1] 	= int(data_sensor[5])
				temp[-1] 	= int(data_sensor[6])
